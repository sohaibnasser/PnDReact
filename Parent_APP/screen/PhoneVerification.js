...
const PhoneVerification = ({ navigation, route }) => {
  ...
  const verifyOTP = async () => {
    if (otp !== responseOtp) {
      return Alert.alert("Incorrect OTP", "Please provide correct OTP");
    }
    try {
      setIsLoading(true);
      ...
      if (responseData.code === "200") {
        ...
        return Alert.alert(
          "Success",
          "OTP is submitted you will be redirected to login screen to login with provided credentials",
      ...
      <View>
        ...
        {otp === responseOtp ? (
          ...
          <View style={styles.smsText}>
            <Text>
              Did't get the SMS <Text> </Text>
              <Text onPress={getOtp} style={{ color: "#49bece" }}>
                {/* {isLoading ? ActivityIndicator : "Send Again"} */}
                Send Again
              </Text>
            </Text>
            <Text style={styles.otp__code}>Your OTP {responseOtp}</Text>
          </View>
        )}
      </View>
